...
- **Master Data** — Company Profile, Category Master, Sub-Category Master (cascading Category → Sub-Category), Type Master, Products (Category/Sub-Category/Type linked via database-driven cascading dropdowns; an optional Opening Balance recorded once at creation — see Inventory below), Customers, Suppliers.
- **Inventory** — Stock Enquiry, Stock Opening (the one place a product's *initial* stock is posted — including a Product's own Opening Balance, entered at Product creation, which is never re-applied on a later edit), Stock Adjustments (book-vs-actual — this is the sanctioned way to correct a product's *current* stock after that initial figure, with its own audit trail; Product Master's Edit screen deliberately cannot touch stock), Stock Transfers (godown to godown), Offcuts (reusable leftover tracking).
